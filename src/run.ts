...
function handleRunner(
  ...
  debug.default(
    `${c.gray(parameters.file)} changed, applying itsss handler...`
  );
  ...
  try {
    ...
    warn(
      PLUGIN_NAME,
      `Handler failed for ${parameters.file}: ${error.message}`
    );
...
function getRunnerName(runner: Runner) {
  return (
    ...
    [getRunnerCommand(runner), ...getRunnerArguments(runner)]
      .join(" ")
      .toLowerCase() ||
    "<runner>"
  );
...
function getRunnerArguments(runner: Runner): string[] {
  const args =
    typeof runner.run === "function" ? runner.run() : runner.run ?? [];
